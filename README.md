...
| `des_mode.py` | Marks where a DES or DDES (SST) run is in LES mode (1) or RANS mode (0), as `Mode_DES` / `Mode_DDES`. `--variant des\|ddes\|both` picks which one to compute. DDES also needs `Turbulent Viscosity` and `Distance to wall`. |
