...
| `des_mode.py` | Marks the RANS and LES regions of an SST-DES or SST-DDES run, following the Fluent Theory Guide. It writes `L_t`, `F_DES` and `LES_mode` (1 = LES, 0 = RANS) with the same names in both modes, and prints the fraction of nodes in LES mode. `--mode des\|ddes` picks the formulation; if it's left out, the script asks. DDES shields with the SST `F2` function and also needs `Distance to wall`. |
