...
| `des_mode.py` | Marks the RANS and LES regions of a hybrid run, following the Fluent Theory Guide. `--mode sa-des\|sa-ddes\|sst-des\|sst-ddes` picks the model (the script asks if it's left out), and `--input FILE` loads a `.plt` / `.szplt` / `.dat` first. Every mode writes `LES_mode` (1 = LES, 0 = RANS) and only the variables that mode needs, then prints the fraction of nodes in LES mode. SA-DDES is shielded with Spalart's `f_d`, SST-DDES with the SST `F2` function. |
